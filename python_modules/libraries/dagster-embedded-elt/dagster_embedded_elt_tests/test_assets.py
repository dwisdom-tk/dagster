...
def test_build_sling_asset(
    ...
    for n in range(runs):
        ...
        assert res.success
        counts = sqlite3.connect(temp_db).execute("SELECT count(1) FROM main.tbl").fetchone()[0]
...
def test_update_mode(temp_db: str, test_csv: str, sling_sqlite_resource: SlingResource):
